...
describe("single(json)", () => {
    ...
    it("should reject when no records match the criteria", async () => {
        // Arrange
        const mockJson = "{}";
        ...
        // Assert
        await expect(promise).rejects.toBe('No element satisfies the condition in predicate');
        expect(mockMatchesFn).toHaveBeenCalledWith(mockRecord);
    });

    it("should reject when there are no records in the database", async () => {
        // Arrange
        const mockJson = "{}";
        const mockMatchesFn = jest.fn();
